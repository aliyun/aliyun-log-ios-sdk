...
(function (global, factory) {
  ...
  (global = global || self, global.OTelJSBridge = factory());
  ...
  ***************************************************************************** */
  ...
  var _OTelJSBridgeFormData = /** @class */ (function () {
       ...
       * 遍历表单记录
       */
      _OTelJSBridgeFormData.traversalEntries = function (formData, traversal) {
          var e_1, _a;
   ...
   * OTelJSBridge 工具
   */
  var OTelJSBridgeUtil = /** @class */ (function () {
      function OTelJSBridgeUtil() {
      }
      /**
       * 把 arraybuffer 转成 base64
       * @param arraybuffer
       */
      OTelJSBridgeUtil.convertArrayBufferToBase64 = function (arraybuffer) {
       ...
       * @param callback
       */
      OTelJSBridgeUtil.convertFormDataToJson = function (formData, callback) {
          var allPromise = [];
          _OTelJSBridgeFormData.traversalEntries(formData, function (key, value, fileName) {
              allPromise.push(OTelJSBridgeUtil.convertSingleFormDataRecordToArray(key, value, fileName));
          });
       ...
       * @param fileName
       */
      OTelJSBridgeUtil.convertSingleFormDataRecordToArray = function (key, value, fileName) {
          return new Promise(function (resolve, reject) {
              var singleKeyValue = [];
       ...
       * @param file
       */
      OTelJSBridgeUtil.convertFileToJson = function (file) {
          return new Promise(function (resolve, reject) {
              var reader = new FileReader();
      ...
      return OTelJSBridgeUtil;
  }());
  /**
   * 处理 iframe 相关
   */
  var OTelJSBridgeIframe = /** @class */ (function () {
      function OTelJSBridgeIframe() {
      }
      /**
       * 分发消息
       * @param messageString
       */
      OTelJSBridgeIframe.dispatchMessage = function (messageString) {
          // 处理有 iframe 的情况
       ...
       * 添加消息监听处理
       */
      OTelJSBridgeIframe.addMessageListener = function () {
          ...
          window.addEventListener('message', function (e) {
              ...
              if (typeof data == "string") {
                  ...
                  if (str.indexOf("messageType") != -1) {
                      window.OTelJSBridge._handleMessageFromNative(str);
                  }
              }
          });
      };
       ...
       * 添加 ajax 消息监听处理
       */
      OTelJSBridgeIframe.addAjaxMessageListener = function () {
          ...
          window.addEventListener('message', function (e) {
              ...
              if (typeof data == "string") {
                  ...
                  if (str.indexOf("ajaxType") != -1) {
                      window._OTelJSBridgeXHR.setProperties(str);
                  }
              }
          });
      };
       ...
       */
      OTelJSBridgeIframe.setupHook = function () {
   ...
   * 建立与 native 的数据通信
   */
  var OTelJSBridge = /** @class */ (function () {
      function OTelJSBridge() {
          this.uniqueId = 1;
          this.callbackCache = {};
       ...
       * @param callback 调用回调
       */
      OTelJSBridge.prototype.callNative = function (module, method, data, callback) {
          var message = {
          ...
          // 发送消息给 Native
          window.webkit.messageHandlers.OTelJSBridgeMessage.postMessage(message);
      };
      /**
       * 用于处理来自 Native 的消息
       * @param callbackMessage 回调消息
       */
      OTelJSBridge.prototype._handleMessageFromNative = function (messageString) {
          ...
          // 处理有 iframe 的情况
          OTelJSBridgeIframe.dispatchMessage(messageString);
      };
      /**
       * 异步调用方法
       * @param module 模块
       ...
       * @param callback 调用回调
       */
      OTelJSBridge.prototype.call = function (module, method, data, callback) {
       ...
       * @param data 数据
       */
      OTelJSBridge.prototype.syncCall = function (module, method, data) {
          function call() {
              var message = {
                  module: module || 'default',
                  method: method,
              ...
              var response = window.prompt("OTelJSBridge", messageString);
       ...
       * @param callback 事件回调
       */
      OTelJSBridge.prototype.on = function (eventName, callback) {
          // 使用数组，支持多个观察者
       ...
       * @param eventName 事件名字
       */
      OTelJSBridge.prototype.off = function (eventName) {
          var obsevers = this.eventCallbackCache[eventName];
   ...
   */
  var _OTelJSBridgeCOOKIE = /** @class */ (function () {
      function _OTelJSBridgeCOOKIE() {
      }
      _OTelJSBridgeCOOKIE.ready = function () {
          window.OTelJSBridge.call(_OTelJSBridgeCOOKIE.moduleName, 'bridgeReady', {});
      };
      // 静态属性和方法
      _OTelJSBridgeCOOKIE.moduleName = 'cookie';
      /**
       * 通过重新定义 cookie 属性来进行 cookie hook
       */
      _OTelJSBridgeCOOKIE.setupHook = function () {
          try {
              ...
              if (cookieDesc && cookieDesc.configurable) {
                  Object.defineProperty(document, 'cookie', {
                      ...
                      get: function () {
                          ...
                          if (window.OTelJSBridgeConfig.ajaxHook && window.OTelJSBridgeConfig.cookieGetHook) {
                              var cookieJson = window.OTelJSBridge.syncCall(_OTelJSBridgeCOOKIE.moduleName, 'cookie', {
                      ...
                      set: function (val) {
                          ...
                          if (window.OTelJSBridgeConfig.cookieSetHook) { // 如果开启 cookie set hook，则需要把 cookie 同步给 Native
                              window.OTelJSBridge.call(_OTelJSBridgeCOOKIE.moduleName, 'setCookie', {
                                  "cookie": val
                              });
                          }
   ...
   * AJAX 相关方法
   */
  var _OTelJSBridgeXHR = /** @class */ (function () {
      function _OTelJSBridgeXHR() {
      }
      // 静态属性和方法
      _OTelJSBridgeXHR.moduleName = 'ajax';
      _OTelJSBridgeXHR.globalId = Math.floor(Math.random() * 100000);
      _OTelJSBridgeXHR.callbackCache = [];
      /**
       * 生成 ajax 请求唯一id
       */
      _OTelJSBridgeXHR.generateXHRRequestId = function () {
          return (new Date).getTime() + "" + _OTelJSBridgeXHR.globalId++; // 时间戳 + 当前上下文唯一id，生成请求id
      };
      /**
       * 给表单生成新的 action
       */
      _OTelJSBridgeXHR.generateNewActionForForm = function (form, requestId) {
          var orignAction = form.action;
          form.action = _OTelJSBridgeXHR.generateNewUrlWithRequestId(orignAction, requestId);
      };
      /**
       * 利用 requestId 生成新的 url
       */
      _OTelJSBridgeXHR.generateNewUrlWithRequestId = function (url, requestId) {
          var orignAction = url;
          ...
          if (/OTelJSBridge-RequestId/.test(orignAction)) { // 防止重复追加 requestId
              aTag.search = aTag.search.replace(/OTelJSBridge-RequestId=(\d+)/, "OTelJSBridge-RequestId=" + requestId);
          }
          else if (aTag.search && aTag.search.length > 0) {
              var s = aTag.search;
              if (/OTelJSBridge-RequestId/.test(s)) { // 防止重复追加 requestId
                  aTag.search = s.replace(/OTelJSBridge-RequestId=(\d+)/, "OTelJSBridge-RequestId=" + requestId);
              }
              else {
                  aTag.search = s + "&OTelJSBridge-RequestId=" + requestId;
              }
          }
          else {
              aTag.search = "?OTelJSBridge-RequestId=" + requestId;
          }
       ...
       */
      _OTelJSBridgeXHR.generateNewOpenUrlWithRequestId = function (url, requestId) {
       ...
       */
      _OTelJSBridgeXHR.isNonNormalHttpRequest = function (url, httpMethod) {
       ...
       * @param body
       */
      _OTelJSBridgeXHR.sendBodyToNativeForCache = function (targetType, target, originMethod, originArguments, request, requestAsync) {
          /*
          ...
          */
          ...
          var cacheCallback = {
              ...
              callback: function () {
                  ...
                  //   target.setRequestHeader("OTelJSBridge-RequestId", requestId);
                  // }
                  if (targetType === "FORM") { // 表单 submit
                      // 发送之前修改 action，让 action 带上 requestId
                      _OTelJSBridgeXHR.generateNewActionForForm(target, requestId);
                  }
                  // 调用原始 send 方法
          ...
          if (requestAsync) { // 异步请求
              // 缓存 callbcak
              _OTelJSBridgeXHR.callbackCache[requestId] = cacheCallback;
              // 发送 body 请求到 native
              window.OTelJSBridge.call(_OTelJSBridgeXHR.moduleName, 'cacheAJAXBody', request, function (message) {
                  // 处理 native 侧缓存完毕后的消息
                  var callbackFromNative = message;
                  var requestId = callbackFromNative.requestId;
                  // 通过请求 id，找到原始 send 方法并调用
                  if (_OTelJSBridgeXHR.callbackCache[requestId]) {
                      var callbackFromNative_1 = _OTelJSBridgeXHR.callbackCache[requestId];
                      ...
                      delete _OTelJSBridgeXHR.callbackCache[requestId];
                  }
              });
              return;
          }
          // 同步请求
          // 发送 body 请求到 native
          window.OTelJSBridge.syncCall(_OTelJSBridgeXHR.moduleName, 'cacheAJAXBody', request);
          // 发送完成后继续请求原始 send 方法
       ...
       * 安装 AJAX Proxy
       */
      _OTelJSBridgeXHR.setupHook = function () {
          /**
           * 只 hook open/send 方法
           */
          ...
          XMLHttpRequest.prototype.open = function (method, url, async, username, password) {
              ...
              var xhr = this;
              // 生成唯一请求id
              xhr.requestId = _OTelJSBridgeXHR.generateXHRRequestId();
              xhr.requestUrl = url;
              xhr.requestHref = document.location.href;
              xhr.requestMethod = method;
              xhr.requestAsync = async;
              if (_OTelJSBridgeXHR.isNonNormalHttpRequest(url, method)) { // 如果是非正常请求，则调用原始 open
                  return originOpen.apply(xhr, args);
              }
              if (!window.OTelJSBridgeConfig.ajaxHook) { // 如果没有开启 ajax hook，则调用原始 open
                  return originOpen.apply(xhr, args);
              }
              // 生成新的 url
              args[1] = _OTelJSBridgeXHR.generateNewUrlWithRequestId(url, xhr.requestId);
          ...
          XMLHttpRequest.prototype.send = function (body) {
              ...
              if (_OTelJSBridgeXHR.isNonNormalHttpRequest(xhr.requestUrl, xhr.requestMethod)) { // 如果是非正常请求，则调用原始 send
                  return originSend.apply(xhr, args);
              }
              if (!window.OTelJSBridgeConfig.ajaxHook) { // 如果没有开启 ajax hook，则调用原始 send
                  return originSend.apply(xhr, args);
              }
              if (!body) { // 没有 body，调用原始 send
                  ...
                  request.value = OTelJSBridgeUtil.convertArrayBufferToBase64(body);
              }
                  ...
                  fileReader.onload = function (ev) {
                      ...
                      _OTelJSBridgeXHR.sendBodyToNativeForCache("AJAX", xhr, originSend, args, request);
                  };
                  ...
                  OTelJSBridgeUtil.convertFormDataToJson(body, function (json) {
                      request.value = json;
                      _OTelJSBridgeXHR.sendBodyToNativeForCache("AJAX", xhr, originSend, args, request);
                  });
                  return;
              ...
              // 发送到 native 缓存起来
              _OTelJSBridgeXHR.sendBodyToNativeForCache("AJAX", xhr, originSend, args, request, xhr.requestAsync);
          };
          /**
           * hook form submit 方法
           */
          ...
          HTMLFormElement.prototype.submit = function () {
              var args = [].slice.call(arguments);
              var form = this;
              form.requestId = _OTelJSBridgeXHR.generateXHRRequestId();
              ...
              if (!window.OTelJSBridgeConfig.ajaxHook) { // 如果没有开启 ajax hook，则调用原始 submit
              ...
              OTelJSBridgeUtil.convertFormDataToJson(formData, function (json) {
                  request.value = json;
                  _OTelJSBridgeXHR.sendBodyToNativeForCache("FORM", form, originSubmit, args, request);
              });
          };
      };
      return _OTelJSBridgeXHR;
  }());

  /// <reference path="../types/index.d.ts" />
  var init = function () {
      if (window.OTelJSBridge) {
          return;
      }
      /**
       * OTelJSBridge 配置
       */
      var OTelJSBridgeConfig = /** @class */ (function () {
          function OTelJSBridgeConfig() {
          }
          /**
           * 默认开启 ajax hook
           */
          OTelJSBridgeConfig.ajaxHook = true;
          OTelJSBridgeConfig.cookieSetHook = true;
          OTelJSBridgeConfig.cookieGetHook = true;
          /**
           * 开启 ajax hook
           */
          OTelJSBridgeConfig.enableAjaxHook = function (enable) {
              if (enable) {
                  OTelJSBridgeConfig.ajaxHook = true;
                  enableFetchHook(true);
              }
              else {
                  OTelJSBridgeConfig.ajaxHook = false;
           ...
           * 开启 cookie set hook
           */
          OTelJSBridgeConfig.enableCookieSetHook = function (enable) {
              OTelJSBridgeConfig.cookieSetHook = enable;
          };
          /**
           * 开启 cookie get hook
           */
          OTelJSBridgeConfig.enableCookieGetHook = function (enable) {
              OTelJSBridgeConfig.cookieGetHook = enable;
          };
          /**
           * bridge Ready
           */
          OTelJSBridgeConfig.bridgeReady = function () {
              _OTelJSBridgeCOOKIE.ready();
              // 告诉 H5 新的 OTelJSBridge 已经 ready
              var OTelJSBridgeReadyEvent = document.createEvent("Events");
              OTelJSBridgeReadyEvent.initEvent("OTelJSBridgeReady");
              document.dispatchEvent(OTelJSBridgeReadyEvent);
          };
          return OTelJSBridgeConfig;
      }());
      // 初始化 OTelJSBridge 并设为全局对象
      window.OTelJSBridge = new OTelJSBridge();
      // 设置 OTelJSBridgeConfig 为全局对象
      window.OTelJSBridgeConfig = OTelJSBridgeConfig;
      // 设置 _OTelJSBridgeXHR 为全局对象
      window._OTelJSBridgeXHR = _OTelJSBridgeXHR;
      // iframe 内处理来自父 window 的消息
      OTelJSBridgeIframe.addMessageListener();
      // 安装 iframe hook： 设置 iframe 的 sandbox 属性
      OTelJSBridgeIframe.setupHook();
      // 安装 formData hook
      _OTelJSBridgeFormData.setupHook();
      // 安装 cookie hook
      _OTelJSBridgeCOOKIE.setupHook();
      // 安装 ajax hook
      _OTelJSBridgeXHR.setupHook();
      // JSBridge 安装完毕
      window.OTelJSBridgeConfig.bridgeReady();
  };
  init();
  var index = window.OTelJSBridge;
