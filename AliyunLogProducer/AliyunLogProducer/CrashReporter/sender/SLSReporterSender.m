...
AliyunLogProducerConfig *logConfig = nil;
...
- (void)initWithSLSConfig:(SLSConfig *)config {
    ...
    logConfig = [[AliyunLogProducerConfig alloc] initWithEndpoint:endpoint project:project logstore:logstore accessKeyID:config.accessKeyId accessKeySecret:config.accessKeySecret securityToken:config.securityToken];
    ...
    client = [[AliyunLogProducerClient alloc]initWithLogProducerConfig:logConfig callback:on_log_send_done];
}
...
- (BOOL)sendDada:(TCData *)tcdata {
    ...
    __block AliyunLog *log = [[AliyunLog alloc] init];
