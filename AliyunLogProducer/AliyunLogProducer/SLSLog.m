...
- (BOOL) putContents: (NSDictionary *) dict {
    ...
    if (!error) {
        ...
        SLSLogd(@"Your NSDictionary is not support convert to JSON, all values will not be added, please check your data.");
    }
...
- (BOOL) putContent: (NSData *) value {
    ...
    if ([data isKindOfClass:[NSDictionary class]]) {
        ...
        SLSLogd(@"Class %@ not support convert to JSON.", [data class]);
        return NO;
    }
...
- (BOOL) putContent: (NSString *) key arrayValue: (NSArray *) value {
    if ([self checkValue:key] && value && [NSJSONSerialization isValidJSONObject:value]) {
        ...
        if (nil != error) {
            SLSLogd(@"error while deserializing NSArray to JSON. error: %@", error.description);
            return NO;
        }
...
- (BOOL) putContent: (NSString *) key dictValue: (NSDictionary *) value {
    if ([self checkValue:key] && value && [NSJSONSerialization isValidJSONObject:value]) {
        ...
        if (nil != error) {
            SLSLogd(@"error while deserializing NSDictionary to JSON. error: %@", error.description);
            return NO;
        }
